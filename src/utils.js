 ...
 */
export async function generateTelemetryHeaders(settings, includeAgentData) {
  const headers = {};
  if(settings.send_telemetry) {
    const agent = await browser.runtime.getBrowserInfo(),
          platform = await browser.runtime.getPlatformInfo();
    if(includeAgentData) headers["Reporting-Agent"] = `${agent.name}/${agent.version} @ ${platform.os}/${platform.arch}`;
